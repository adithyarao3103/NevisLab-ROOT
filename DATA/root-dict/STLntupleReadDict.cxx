...
// Read the ntuple created with STLntupleCreateDict. Here, we're using
// a compiled dictionary (as opposed to STLntupleCreate.cxx, which
// uses a dynamically-generated dictionary).

// To compile this program:
// g++ `root-config --cflags --libs` STLntupleReadDict.cxx ./STLntupleDict.so -o STLntupleReadDict
...
void STLntupleRead() 
{
  ...
  auto input = new TFile("STLntupleDict.root");
  ...
  auto output = new TFile("STLntupleHistDict.root","RECREATE");
  ...
  while (myReader.Next()) {
    ...
    for ( const auto& [key, value] : *map2D ) { 
        // Recall that map's kkey is a tuple, so we can use std::get to access its elements.
